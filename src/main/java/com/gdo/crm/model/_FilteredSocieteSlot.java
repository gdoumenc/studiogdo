...
public abstract class _FilteredSocieteSlot extends SocieteSlot {

	protected _Filter _cond;

	public _FilteredSocieteSlot(StclContext stclContext, Stcl in, String name) {
		super(stclContext, in, name);
		this._cond = getFilter();
	}

	public SocieteSlot getParentSlot(StclContext stclContext, PSlot<StclContext, PStcl> self) {
		PStcl container = self.getContainer();
		return container.getResourceSlot(stclContext, Resource.SOCIETE).getSlot();
	}
