    ...
    private class FilesOnlySlot extends MultiCalculatedSlot<StclContext, PStcl> {
        ...
        protected StencilIterator<StclContext, PStcl> getStencilsList(StclContext stclContext, StencilCondition<StclContext, PStcl> cond, PSlot<StclContext, PStcl> self) {
            try {
                ...
                if (folder == null)
                    return StencilUtils.< StclContext, PStcl> iter(Result.error("cannot connect"));
                ...
                return StencilUtils.< StclContext, PStcl> iter(stclContext, list.iterator(), cond, self);
            } catch (Exception e) {
                logError(stclContext, e.toString());
                return StencilUtils.< StclContext, PStcl> iter(Result.error(e.getMessage()));
            }
        }
    }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        return StencilUtils.< StclContext, PStcl> iter(Result.error("cannot get mail folder"));
                    }
                }
