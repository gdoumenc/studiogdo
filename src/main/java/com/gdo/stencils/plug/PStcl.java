...
public class PStcl extends _PStencil<StclContext, PStcl> {
	...
	public void initialize(StclContext stclContext, PStcl pstencil, PSlot<StclContext, PStcl> slot, IKey key) {
		if (pstencil.isCursorBased()) {
			...
			initialize(stclContext, pstencil.getReleasedStencil(stclContext), slot, key);
		}
	}
    ...
    public String getValue(StclContext stclContext) {
        ...
        if (isCursorBased()) {
            return getId(stclContext);
        }
        
        // gets value from complete structure
        return super.getValue(stclContext);
    }
	...
	public String getResourceValue(StclContext stclContext, String resource, String def) {
		String value = getResourceStencil(stclContext, resource).getValue(stclContext);
	...
	public void setResourceValue(StclContext stclContext, String resource, String value) {
		getResourceStencil(stclContext, resource).setValue(stclContext, value);
	}
	...
	/**
	 * Returns the path of some resources depending on the project configuration.
	 * 
	 * @param stclContext
	 *          the stencil context.
	 * @param resource
	...
	public void addPropertyValue(StclContext stclContext, String path, String value) {
		...
		// search string in slot
		super.setValue(stclContext, value);
	}
    ...
    public PStcl plug(StclContext stclContext, PStcl stencil, String slotPath, IKey key) {
        if (_Stencil.Slot.$LOCKED.equals(slotPath) && isCursorBased()) {
	...
	@Override
	public InputStream getInputStream(StclContext stclContext, String path) {
		return getStencil(stclContext, path).asProp(stclContext).getInputStream(stclContext);
	}
