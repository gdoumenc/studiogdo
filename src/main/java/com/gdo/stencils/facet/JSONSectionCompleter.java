...
public class JSONSectionCompleter extends HTML5SectionCompleter {
	...
	private List<HashMap<String, String>> expandToJSON(StclContext stclContext, PStcl stcl, Parameters param) {

		// sets values
		String[] columns = PathUtils.splitMultiPath(param.mode);
		StencilIterator<StclContext, PStcl> iter = stcl.getStencils(stclContext, param.path);
		List<HashMap<String, String>> result = new ArrayList<HashMap<String, String>>(iter.size());
		for (PStcl s : iter) {
			HashMap<String, String> row = new HashMap<String, String>();
			result.add(row);
			row.put("key", s.getKey().toString());
			row.put("path", s.pwd(stclContext));
			row.put("name", s.getUId(stclContext));
			row.put("id", s.getId(stclContext));
			for (String att : columns) {
				String value = s.getString(stclContext, att, null);
				row.put(att, value);
			}
		}
	...
	private JsonElement expandPathObject(StclContext stclContext, PStcl stcl, JsonObject object) {
		...
		String valuePrefix = getAttribute(object, VALUE_PREFIX_ATTRIBUTE);
		String valueSuffix = getAttribute(object, VALUE_SUFFIX_ATTRIBUTE);
...
//				JsonElement replacement = expandObject(stclContext, child, member.getAsJsonObject());
//				result.add(replacement);
			}
		}
		return result;
